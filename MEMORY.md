...
**Palette (white + forest green).** Klaus ships in a strict two-tone palette: pure white `(255, 255, 255)` for body panels / face screen / highlights, and forest green `(34, 100, 55)` for outlines, eyes, antenna stem, and detail pixels. There is **no brown, orange, red, or blue** anywhere in the mascot — those colors were in the original artist-supplied source and were remapped at asset-generation time. The recolor is a simple luminance split in `recolor_frame` (`scripts/generate_klaus_frames.py`): after the brown backdrop is knocked to `alpha=0`, every remaining pixel with luminance (`0.299·R + 0.587·G + 0.114·B`) ≥ **195** becomes white, else forest green. The threshold was raised from an earlier `130` to `195` after the lower value was found to collapse mid-tone face details (pink cheeks at luminance ~178, red accent squares, orange antenna tips) into the same bucket as the brightest body panels and TV-screen face, flattening the face to an almost featureless white. At 195 only the brightest body / face-screen pixels stay white, while cheeks, eyes, mouth dot, and antenna tips all land in forest green so the face reads with its full expression. No runtime color inversion is used — `invertColors` defaults to `false` and the baked-in palette is what you see.
