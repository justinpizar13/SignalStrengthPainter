...
| **`PaywallView.swift`** | NetSpot-inspired Pro upsell screen: Canvas hero, pricing cards (**Monthly $3.99**, **Yearly ~~$39.99~~ $34.99** with "Best Deal" badge), Buy Now CTA, Restore / Not Now links, X close. **Default selection is Yearly** (was Monthly) so trial users land on the annual plan. **Trial-aware copy:** when the currently-selected product is intro-offer eligible, the CTA flips from "Buy Now" to **"Start 3-Day Free Trial"**, the Yearly card subtitle becomes "3 days free, then $34.99/year", a **trial timeline** block ("Today: Full access → Day 2: Charge reminder → Day 3: Trial ends") is rendered under the CTA, and a `disclosureLine` ("Cancel anytime in Settings — trials auto-renew if not cancelled") is shown. Accepts optional `onPurchase` closure for tab-mode integration. Backed by `ProStore` (StoreKit 2) — Buy Now calls `ProStore.purchase(_:)`, Restore calls `AppStore.sync()`, and `isProUser` flips only when `Transaction.currentEntitlements` contains a verified, non-revoked entry. |
