...
| **`KlausMascotView.swift`** | Animated pixel-art mascot used across the Klaus chat experience. Loads the `KlausMascot` data asset (a transparent animated GIF shipped in `Assets.xcassets`), decodes frames + per-frame delays with `ImageIO`, and renders them through a `UIImageView`-backed `UIViewRepresentable` with nearest-neighbor `magnificationFilter`/`minificationFilter` so pixel-art detail stays crisp at any size. Frame list cached once per process behind an `NSLock`. The `size` parameter is the **bounding-box side length** (mascot height equals `size`; width is derived from the 336×446 aspect ratio), so Klaus always fits inside a `size × size` square — previously `size` was the width, which made the mascot render ~33% taller than its container and caused it to overflow avatar circles and dominate the assistant header. An `invertColors: Bool = true` parameter applies SwiftUI `.colorInvert()` to the rendered frames so Klaus's body colors flip (whites → blacks, blues → orange, etc.) while transparent edges stay transparent — the underlying GIF asset is untouched, so flipping the flag off restores the original palette without re-running `generate_klaus_frames.py`. The inversion is wrapped in a private `KlausColorModifier` `ViewModifier` so the `UIImageView` identity is preserved across toggles and the animation state isn't reset. Used in the assistant header (44 pt), each assistant chat bubble avatar (30 pt clipped to a 34-pt circle), and the blue-gradient CTA card on the Signal tab (40 pt in a 46-pt circle). |
...
**Klaus** is the pixel-art astronaut-robot mascot that fronts the Wi-Fi Buddy assistant. He lives in `KlausMascotView.swift` and replaces the generic "Ask Wi-Fi Buddy" assistant branding so users feel like they're chatting with a *character* rather than a form. Visual identity is a chunky pixel-art robot with a TV-screen face and a stubby antenna (the source asset the user provided is an animated GIF — 38 frames of idle-breathing animation).

**Asset pipeline:** `scripts/generate_klaus_frames.py` (Pillow) takes the source GIF at `/Users/jupizarr/.cursor/projects/Users-jupizarr-SignalStrengthPainter/assets/polifolli-robot-*.png`, sweeps every frame to compute a union bounding box of the robot (excluding the brown `(153, 125, 118)` backdrop with a tolerance of 18), pads by 8 pixels, crops each frame to that box (336 × 446 pixels), and knocks out the brown backdrop to transparent pixels so the mascot sits cleanly on any themed card. Output goes to `SignalStrengthPainter/Assets.xcassets/KlausMascot.dataset/klaus.gif` as a transparent animated GIF (~155 KB, looped indefinitely, per-frame delays preserved from the source).

**Data asset vs imageset:** Klaus is shipped as an `NSDataAsset` (not an imageset) because SwiftUI's `Image` does not play animated GIFs. The `.dataset/Contents.json` declares the universal-type-identifier `com.compuserve.gif` so the asset catalog ships the raw GIF bytes verbatim, letting runtime code decode frames via `ImageIO`.

**`KlausMascotView.swift`** — two pieces:

1. **`KlausMascotAssets`** — singleton loader that decodes the GIF once per process via `CGImageSourceCreateWithData`, extracts every frame as a `UIImage`, reads the per-frame delay from `kCGImagePropertyGIFDictionary` (`kCGImagePropertyGIFUnclampedDelayTime` preferred, clamped to a `0.02` s minimum so malformed 0-delay frames don't burn CPU), caches the result behind an `NSLock`, and exposes an `AnimationPayload` of `frames + duration + pixelSize`. Static constant `aspectHeightOverWidth = 446.0 / 336.0` keeps the SwiftUI frame proportional to the underlying pixel art. `KlausMascotView(size:)` uses this ratio to treat `size` as the **bounding-box side** — height = `size`, width = `size / aspectHeightOverWidth` — so the mascot fits inside a `size × size` square and Klaus no longer overflows the 46 pt "Chat with Klaus" avatar circle or dominates the 44 pt assistant header (pre-fix, `size` was the width, so Klaus rendered at `size × 1.33·size` and overshot every container).
2. **`KlausAnimatedImage`** (`UIViewRepresentable`) — wraps `UIImageView` so the frames can actually animate (`animationImages`, `animationDuration`, `animationRepeatCount = 0`). Critically, the image view's layer has `magnificationFilter = .nearest` and `minificationFilter = .nearest` so the chunky pixels stay crisp when scaled up/down — no interpolation blur. `isAnimating` is toggleable (the chat uses `true` everywhere; a future "Klaus is sleeping" state could flip it off).

**Where Klaus appears:**
- **`WiFiAssistantView`** header — `KlausMascotView(size: 44)` replaces the old `AppLogoView`, with title "Klaus" and subtitle "Your Wi-Fi Buddy sidekick".
- **`WiFiAssistantView`** chat bubbles — assistant avatar is now Klaus clipped to a 34-pt circle with a soft blue `0.12` fill behind him (replacing the previous SF Symbol `wifi` glyph). The 30-pt Klaus is offset +1 pt vertically because the robot's center of mass sits slightly above his feet within the bounding box.
- **`MainTabView` → `SignalDetailView` assistantCTA** — the blue-gradient entry point card on the Signal tab now reads "Chat with Klaus" + "Your Wi-Fi sidekick, always on call" with a 40-pt Klaus in a 46-pt white-tinted circle.
- Input placeholder changed from "Ask about your Wi-Fi..." to "Ask Klaus about your Wi-Fi..." so the input bar reinforces the character framing.
