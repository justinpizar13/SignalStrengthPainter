...
| **`AboutView.swift`** | **About & Guide sheet (April 2026).** Static, read-only reference screen reached from a hamburger button in the top-left of the Speed-tab Wi-Fi header. Stacks themed cards: hero (logo + tagline), "What is Wi-Fi Buddy?", a five-tab walkthrough (Speed / Survey / Signal / Devices / Pro — same icon + copy vocabulary as the real tab bar so users can match what they read to what they see), step-by-step how-tos for **Run a Speed Test** and **Survey Your Home**, a Klaus introduction with the mascot avatar, a Pro feature summary, a privacy card ("your data stays on this device"), and a version footer read from `CFBundleShortVersionString` / `CFBundleVersion`. Presented via `.sheet(isPresented:)` with `NavigationStack` + inline title and a circular X close button in the top-trailing toolbar slot so the dismiss affordance matches the rest of the app's sheets. All copy is in-file so wording tweaks don't touch other views. Completely static — no network probes, no state beyond the dismiss action — so this sheet is cheap to push and never blocks the rest of the UI. |
