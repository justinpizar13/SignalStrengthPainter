...
## Marketing website (`website/`, April 2026)

A static marketing site for WiFi Buddy lives in **`website/`** at the repo root, alongside the iOS code. Built with **Astro 5** + **Tailwind v4** + **MDX**. The intent is small and SEO-first: feed GEO citations and capture purchase-intent queries like "best wifi heatmap app for iphone" before the user opens the App Store.

**Stack & integrations:**
- Astro 5 (`^5.18.1`) + `@astrojs/mdx@^4` + `@astrojs/sitemap@^3` + `@tailwindcss/vite@^4` + `tailwindcss@^4`. Astro 6 was tried first but its Rolldown bundler currently breaks `@tailwindcss/vite` with a `Missing field tsconfigPaths on BindingViteResolvePluginConfig.resolveOptions` error during build, so we pinned to Astro 5 — there's no functional gap for a static marketing site and the integration is rock-solid.
- Output is pure static HTML/CSS in `website/dist/` plus a tiny **~600-byte reveal-on-scroll script** inlined into each page (added April 29, 2026 — see "Motion polish" note below). Builds in ~1.0–1.3 s on M-series hardware. 16 routes total.

**Routes (all under `website/src/pages/`):**
- `/` (`index.astro`) — landing page. Hero on the left (tagline + App Store CTA + "How it works" link), a real iPhone screenshot of the survey-insights screen on the right (`public/screenshots/survey-results.png` in a phone-frame div with rounded `2.25rem` corners). Below the hero: a 6-card feature grid (AR survey, insights engine, identifies every device, real speed test, privacy by construction, live network topology — Klaus is pulled out of the grid into its own dedicated section), a **dedicated Klaus section** featuring `klaus-chat.png` beside the heading **"Klaus, your AI Wi-Fi sidekick."** (reworded from "offline Wi-Fi sidekick" in the April 2026 launch refresh), an "A quick tour" section with four phone-framed screenshots (Speed / Survey / Signal / Devices), and a final "The whole home, one walk" CTA card linking to `/releases` with "What's new in 1.0 →". Uses `hero-glow` body class for the radial blue accent.
- `/features` — AR survey + `SurveyInsightsEngine` deep dive. Mirrors the iOS architecture vocabulary (calibration stages, A–F grading, dead-zone clustering at 1.8 m, jitter/median ratio, router-direction Pearson correlation). Klaus subsection heading also reworded to "Klaus, your AI Wi-Fi sidekick".
- `/devices` — the **8 identification layers** explained one-per-card (Bonjour, TCP port fingerprinting, TCP liveness probe, SSDP/UPnP, UPnP description fetch, reverse DNS, ARP table read, OUI lookup). Carries a `TechArticle` JSON-LD on top of the default `MobileApplication` schema.
- `/releases` (April 2026 launch refresh) — release-notes timeline. `releases.astro` defines a `Release[]` array (newest-first) where each release has `version`, ISO `date`, optional `tagline`, and one or more `ChangeGroup` sections (`What's new` / `Pro & pricing` / `Privacy & platform`, etc., each with a traffic-light accent color matching the rest of the brand). Adding a future 1.1 entry is a one-object prepend at the top of `releases`. Replaced the `/vs-*` comparison pages, which were removed in the same refresh — comparison content was generating low-quality "X vs Y" copy that risked App Store review concerns about disparaging competitors and didn't fit the indie-launch narrative.
- `/blog/` (index) + 9 evergreen MDX posts under `src/content/blog/`:
  - `fix-wifi-dead-zones.mdx`
  - `jitter-and-video-calls.mdx`
  - `find-unknown-devices-on-wifi.mdx`
  - `mac-randomization-ios.mdx` (linked from `/devices`)
  - `speed-test-vs-isp-plan.mdx`
  - `mesh-vs-extender-vs-powerline.mdx` (linked from `fix-wifi-dead-zones`)
  - `best-wifi-heatmap-apps-iphone.mdx`
  - `best-router-placement.mdx`
  - `2-4-vs-5-ghz.mdx`
- `/privacy`, `/terms` — render the canonical Markdown verbatim. **Do not edit `website/src/content/legal/*.md` directly** — those files are auto-generated. Edit the canonical files at the repo root (`PrivacyPolicy.md` and `SignalStrengthPainter/TermsOfUse.md`); a `predev` / `prebuild` npm hook runs `scripts/sync-legal.mjs` which `copyFileSync`s them into the website's content folder. This guarantees the in-app `LegalDocumentView`, the App Store-bundled markdown, and the public web URLs all serve byte-identical text — App Store reviewers can audit one against the other.

**Layouts & components:**
- `BaseLayout.astro` — the SEO chassis. Renders `<title>`, `<meta description>`, canonical URL, OG/Twitter cards, theme color, optional `noindex`, **and a `MobileApplication` JSON-LD on every page** with `applicationCategory: "UtilitiesApplication"`, `operatingSystem: "iOS 17.0"`, both subscription `Offer`s, and an optional `aggregateRating` block. Pages can pass extra `jsonLd` (e.g. `/devices` adds `TechArticle`, blog posts add `Article`). Smart title logic: pages whose title already contains "WiFi Buddy" don't get the brand appended a second time (this used to be exercised by the `/vs-*` comparison pages, which are gone post-April 2026 launch refresh — the smart-title behavior is still useful for any future page that wants to bake the brand into the H1).
- `PostLayout.astro` — wraps blog post body in the `.prose` typography class, adds `Article` JSON-LD with `datePublished` / `dateModified` / `mainEntityOfPage`, plus a "from the makers of" CTA card and a back-to-blog link.
- `Header.astro` / `Footer.astro` / `Section.astro` / `FeatureCard.astro` / `AppStoreButton.astro` — small focused components. None of them ship their own client JS; the only client script on the site is the reveal-on-scroll observer inlined in `BaseLayout.astro` (see "Motion polish" below). (Pre-launch the components folder also held `CompareTable.astro` and `CompareNav.astro` powering the `/vs-*` pages; both were deleted in the April 2026 launch refresh along with the `Compare` primary-nav entry and the `/vs-*` `isActive` branch in `Header.astro`. The `Compare` slot in `PRIMARY_NAV` was replaced by **`Releases`**, and the footer's three-column "Compare" group was collapsed into a smaller "Company" column with Release notes + Contact.)
- `AppStoreButton.astro` — renders Apple's **official "Download on the App Store" badge artwork** (`public/app-store-badge.svg`, sourced unmodified from Apple's marketing-resources kit, US/UK black lockup) wrapped in a link to `SITE.appStoreUrl`. Badge sizes map to `sm: 40 px`, `md: 48 px`, `lg: 56 px` — even the smallest height respects Apple's documented 40-px minimum. Width is computed from the badge's intrinsic 119.66 × 40 viewBox (`Math.round(h * 119.66407 / 40)`) so the aspect ratio is never stretched. The wrapping anchor only adds hover/active opacity feedback and the `aria-label` "Download WiFi Buddy on the App Store"; **no recoloring, no border, no rounded corners, no overlay** — Apple's marketing guidelines explicitly forbid modifying the artwork. The earlier homemade variant (custom Apple-glyph SVG + "Download on the / App Store" two-line text stack on a white pill) was replaced in the April 2026 launch refresh because using the official badge is required by App Store Marketing Guidelines and avoids any review-time pushback. The component's public `Props` interface stayed source-compatible (`size`, `class`); only the unused `label` override was dropped because the official badge is a fixed image. All five callsites (`index.astro`, `features.astro`, `releases.astro`, `devices.astro`, `PostLayout.astro`) compile unchanged.
- `Logo.astro` — **brand mark, geometry mirrors `AppLogoView.swift` exactly**: `cy = 0.695`, arc bbox radii `0.445 / 0.315 / 0.185`, stroke `0.080`, dot `0.055`, arc sweep 225° → 315°, traffic-light palette (`#4DD966 / #FAD138 / #FA8538` arcs + `#FA5252` dot). `withTile` prop wraps it in the dark-gray `#28282C` squircle to match the home-screen icon. The arc paths are computed in JS at component-render time using the same constants — if `AppLogoView.swift` ever moves, update `Logo.astro` to match.

**Brand tokens (`src/styles/global.css`):** `@theme { --color-bg ... }` block declares the dark surface stack and the same five signal colors as the iOS app. Tailwind v4 picks them up natively. The `.prose` class is hand-tuned for blog/legal markdown rather than using `@tailwindcss/typography` — keeps the bundle smaller and gives us exact control over the dark palette.

**Single source of truth — `src/lib/site.ts`:** App Store URL, domain, support email, pricing, optional aggregate rating, primary nav, and footer nav. Every page imports from here, so changing the App Store ID once it's live is a one-line edit. Pricing tokens (`SITE.pricing.monthly`, `SITE.pricing.yearly`) are also wired into the `MobileApplication` JSON-LD so the offers in structured data match the marketing copy. The `COMPARE_NAV` constant + the "Compare" entry in `PRIMARY_NAV` were both removed in the April 2026 launch refresh; "Releases" replaced "Compare" in `PRIMARY_NAV` and was added to `FOOTER_NAV` so users land on what's-new instead of competitor comparisons.

**SEO scaffolding wired up:**
- One `<h1>` per page, per-page `<title>` and `<meta description>`, canonical URLs, OG/Twitter cards on every route.
- `MobileApplication` JSON-LD on every page (Google Knowledge Panel + AI overviews); `Article` JSON-LD on every blog post; `TechArticle` JSON-LD on `/devices`.
- `/sitemap-index.xml` + `/sitemap-0.xml` auto-generated by `@astrojs/sitemap` (16 entries after the April 2026 launch refresh — `/vs-*` pages out, `/releases` in; was 18 entries before).
- `public/robots.txt` allow-listing GPTBot, Google-Extended, PerplexityBot, ClaudeBot in addition to the default `User-agent: *`. Sitemap URL is referenced at the bottom.
- App Store CTAs (every page) route through the canonical `apps.apple.com/app/wifi-buddy/idTODO` URL — Apple gets attribution, install completes inside the App Store. The placeholder `idTODO` lives in `SITE.appStoreUrl` and is the one thing that must change before launch.
- `public/favicon.svg` and `public/og-default.svg` are hand-authored SVGs that mirror the `AppLogoView` geometry — same arc radii, same palette, same `cy`. The OG card is a 1200×630 social card with the dark-gray tile, the brand mark, the tagline, and a generic "Download on the App Store" rectangle.
- `public/app-store-badge.svg` — Apple's official "Download on the App Store" black-lockup badge, sourced from Apple's marketing-resources kit. Served unmodified by `AppStoreButton.astro` (see component description above).
- `public/screenshots/` — nine real iPhone screenshots (each 472×1024, the captured device width × height) added in the April 2026 launch refresh: `survey-results.png` (used in the homepage hero), `klaus-chat.png` (Klaus dedicated section on `/`), `survey-walk.png`, `speed-test.png`, `signal-tab.png`, `devices-tab.png` (the four "A quick tour" tiles on `/`), plus `speed-report.png`, `getting-started.png`, and `getting-started-klaus.png` held in reserve for future hero swaps or blog inline imagery. All nine are PNGs scaled to 472-px-wide previews — the original retina-resolution captures aren't needed for web because each screenshot is only ever rendered at ≤ 280 px wide inside its phone-frame container. When swapping screenshots after a UI change, prefer keeping the same filenames so the homepage and tour markup don't need updates; if a tab gets a new name, update the `tour` array in `index.astro` (it's a typed `Array<{ src, alt, label, caption }>` defined at the top of the file).

**Klaus rebrand (April 2026 launch refresh):** the Klaus heading on both `/` and `/features` was reworded from **"Klaus, your offline Wi-Fi sidekick"** → **"Klaus, your AI Wi-Fi sidekick"** to align with the in-app paywall page-3 copy ("Klaus AI Assistant"). The body copy on `/features` still describes Klaus accurately as a curated keyword-scored knowledge base (no LLM, no network calls) — only the marketing-tagline was updated, since "AI" in App Store / utility-app vernacular increasingly maps to "answers your question in plain English" rather than implying a hosted LLM. The blog copy in `PostLayout.astro` and `best-wifi-heatmap-apps-iphone.mdx` still uses "offline assistant" as a *technical-attribute* description of Klaus and was intentionally left alone — those phrasings are about the implementation (runs on-device, no network), not the brand subtitle.

**Motion polish (April 29, 2026):** the site felt static after launch, so a small layer of motion was added entirely through `src/styles/global.css` plus one inlined script in `BaseLayout.astro`. No new components, no animation library. Concretely:
- **Animated hero glow** — `.hero-glow` (the body class on `/`) gained a 22 s `ease-in-out alternate` `hero-glow-drift` keyframe that nudges the radial-gradient `background-position` a few percent in each axis, so the hero feels alive without a visible loop seam. Two stops are layered (signal-blue ellipse near top center + signal-green ellipse upper-right) for subtle color shift.
- **`.float-slow`** — 7 s vertical bob (`translateY(0) ↔ -10px`) used on the hero phone screenshot in `index.astro`. Opt-in via the class.
- **`.card` hover** — the shared card component now lifts 3 px, brightens its background via `color-mix(in oklab, var(--color-bg-card) 92%, white)`, and gets a soft signal-blue border + outer shadow on hover. Wrapped in `@media (hover: hover)` so touch devices don't get sticky hover state. `FeatureCard.astro` got a `feature-icon` class on its accent tile so `.card:hover .feature-icon { transform: scale(1.08) rotate(-3deg) }` makes the whole card feel like one tactile unit.
- **`.nav-link`** — primary nav links got a `::after` pseudo-element underline that slides in via `transform: scaleX(0 → 1)` on hover/focus and stays expanded for the active route (`.nav-link.is-active`). `Header.astro` was updated to add `nav-link` + conditional `is-active`.
- **`.cta-press`** — small utility for App Store buttons + ghost CTAs: `translateY(-1px)` on hover, back to 0 on `:active`. Applied to `AppStoreButton` callsites and the "How it works" / "Email support" ghost buttons.
- **Reveal-on-scroll** — the centerpiece. CSS defines two opt-in attributes: `[data-reveal]` (single element fades up + translates from `+20px` over 0.8 s) and `[data-reveal-stagger]` (children of the container animate in 60 ms apart, up to 8 children — `:nth-child(1)..(8)` `transition-delay` rules). All hidden states are gated on `html.js`, which is set by an inline `<script is:inline>` in `<head>` *before first paint* — so crawlers, no-JS visitors, and `prefers-reduced-motion` users see fully rendered content with zero FOUC. A bundled `<script>` at the bottom of `<body>` runs a single `IntersectionObserver` (`rootMargin: "0px 0px -8% 0px"`, `threshold: 0.05`) that adds `is-visible` once per element and disconnects. Falls back to "show everything immediately" if `IntersectionObserver` is missing or reduced motion is requested. Astro inlines the bundled script (~600 bytes minified) into each page rather than emitting a separate `.js` file — verified in `dist/index.html`.
- **`prefers-reduced-motion: reduce`** — a single `@media` block at the bottom of `global.css` cancels every animation, transition, and reveal hidden state. This is non-negotiable for accessibility.
- **Where reveals are sprinkled:** **above-the-fold hero blocks deliberately have NO reveal markers** so the H1 + intro paragraph + hero phone screenshot render instantly with zero flash (this caught me on the first pass — the brief opacity:0 between first paint and the deferred IO callback firing made it feel like screenshots had vanished on slow loads). Concretely: `index.astro` only reveals the feature grid stagger, Klaus card, tour grid stagger, and final CTA card; the hero phone keeps its `float-slow` class but ships visible. `features.astro` reveals the AR-survey two-col stagger, insights three-card stagger + summary card, speed/topology two-col stagger, Klaus card, and final CTA. `devices.astro` reveals the 8-layer `<ol>` stagger, MAC + trust two-col stagger, and final CTA. `releases.astro` reveals each release `<li data-reveal>` and the feedback card. `blog/index.astro` reveals the post grid stagger. `PostLayout.astro` reveals the "from the makers of" card. **Privacy and Terms pages were intentionally left untouched** so dense legal prose doesn't fade in while someone is scanning it. As a defense-in-depth, the IO bootstrapper in `BaseLayout.astro` runs `getBoundingClientRect()` synchronously on every `[data-reveal]` / `[data-reveal-stagger]` it finds and marks any element already on screen `is-visible` immediately — only off-screen elements are handed to the observer. So even if someone re-adds `data-reveal` to a hero in the future, the page won't flash.

**Mobile nav + marketing-focused copy refresh (April 29, 2026):** two issues surfaced once the site went live: (a) the primary nav (`Features / Devices / Releases / Blog`) was rendered with `hidden md:flex` on the `<nav>` and was therefore *completely* missing on mobile — there was no hamburger fallback, so a phone visitor saw the brand mark and "Get the app" CTA but had no way to reach `/features` or `/blog`; (b) the marketing copy across `/`, `/features`, `/devices`, and the v1.0 entry on `/releases` read like an internal architecture deck — it named protocols (Bonjour, SSDP/UPnP), specific TCP ports (62078, 548, 22, 9100, 7000, 1883…), Apple frameworks (`ARWorldTrackingConfiguration`, ARKit, Core Motion), kernel calls (`sysctl(NET_RT_FLAGS)`, `getnameinfo`), the exact cluster threshold (1.8 m), the score weighting (60% coverage / 40% median, p95 penalty), the Pearson-correlation router-direction test, the "deterministic keyword scorer over hand-written answers" framing of Klaus that explicitly disclaims the AI label, and the "8-layer scanner" architecture. This was both a marketing problem (engineering-deck tone vs. App Store-shopper tone) and a competitive-moat problem — anyone could read the website and have a recipe for the app's identification stack.

Fixes:
- **Mobile nav** — `Header.astro` rewritten to add a hamburger `<button id="mobile-menu-toggle">` (visible only below the `md` breakpoint via `md:hidden`) on the right of the brand row, plus a slide-down `<div id="mobile-menu" class="md:hidden hidden">` drawer below the header that renders the same `PRIMARY_NAV` items at touch-friendly 44 pt tap targets and an "Get the app" CTA. The toggle script (a small inline `<script>` at the end of the component) handles open/close, swaps the menu/X icons, sets `aria-expanded` / `aria-label` to match state, dismisses on Escape and on link tap (so anchor navigation still feels instant), and auto-closes when the viewport crosses the `md` breakpoint via `matchMedia("(min-width: 768px)").addEventListener("change", ...)` so a user who rotates a tablet doesn't end up with a stuck-open mobile drawer over the visible desktop nav. Desktop nav (`md:flex`) is untouched. The toggle is the *only* second client script on the site (the first being the reveal-on-scroll observer in `BaseLayout.astro`); both ship inline as Astro auto-bundles them per page.
- **Copy rewrite** — every public-facing marketing page was rewritten so it describes **the outcome the user gets**, not **the mechanism behind it**. The pattern is: keep the user-visible promise (graded report, named dead zones, tailored fixes, real speed test, friendly device list, Klaus chat), drop everything that names the implementation. Specifically:
  - `index.astro` — feature-grid card titles changed: "AR walk-and-paint survey" → "Walk-and-paint Wi-Fi survey" (no AR), "Insights engine, not just dots" → "A real report, not just dots", "Identifies every device" → "Know what's on your Wi-Fi", "Real speed test, real ISP context" → "A speed test you can trust" (the `AZ → DFW` Cloudflare-colo failure-mode anecdote in the body was removed), "Privacy by construction" → "Privacy by design". Klaus section heading went from "Klaus, your AI Wi-Fi sidekick" to "Klaus, your friendly Wi-Fi sidekick" — *intentional softening* because "AI sidekick" was inviting LLM expectations the app doesn't try to meet. Klaus bullets reworded from artifact-counts ("24 hand-written Wi-Fi answers") to attributes ("Clear, no-jargon answers"). The "Free accounts get one question per install" line was loosened to "Free accounts get a question to try" so the exact free-tier limit is no longer hard-coded into website copy (it's still enforced in `WiFiAssistantView`'s `freeMessageLimit`, but gives us room to tune without touching the marketing site).
  - `features.astro` — the "Why AR" sidebar (which referenced ARKit's visual-inertial odometry, Apple's Measure app, and Core Motion's pedometer) became "Designed for indoors / Accurate room-scale tracking" — same accuracy claim, no framework name. "Insights Engine: a real report, not a heatmap" + the engine-name boldface was dropped; the three step cards (Score & grade, Cluster dead zones, Recommend) now read "Score & grade / Find dead zones / Tell you what to do" with descriptions that omit the weighting formula and clustering threshold. "Insights the engine surfaces" → "What's in your report"; the bullet that exposed the p95-vs-median branch was rewritten to "Catches the rooms where your signal mostly feels fine but occasionally drops calls", the jitter-ratio threshold bullet was rewritten as "Stability check" without numbers, and the Pearson-correlation explanation was rewritten as "Router direction hint" without the |r| ≥ 0.35 cutoff. The Cloudflare-colo paragraph in the speed-test section was rewritten to talk about "the way your apps actually use your connection" instead. The Klaus paragraph that explicitly stated "He doesn't call an LLM and he doesn't make a single network request: it's a deterministic keyword scorer over hand-written answers" was deleted in full — the on-device privacy claim is preserved by the dedicated privacy bullet on the homepage, and the clinical disclaimer was actively *hurting* perceived value.
  - `devices.astro` — **the most aggressive rewrite**. The original page was an `<ol>` of 8 `<li>` cards, each one fully naming an identification layer (Bonjour service types like `_airplay._tcp` / `_homekit._tcp` / `_companion-link._tcp`, the 21-port TCP fingerprint list with port numbers and what each port maps to, TCP RST timing under 500 ms, M-SEARCH multicasts to `239.255.255.250:1900` with three different ST values 100 ms apart, UPnP `<friendlyName>` parsing, `getnameinfo` reverse DNS, raw `sysctl(NET_RT_FLAGS)` ARP-table reads parsing the packed `rt_msghdr` struct, ~300-vendor OUI database with named brands). All 8 cards plus the entire `<ol>` are gone. Replaced with a 3-card outcome grid: **What you see / Why it works / No router login**. The MAC-randomization section became "Modern privacy features, handled" (no longer names the locally-administered-bit detection); the trust section kept the user-visible flow but removed the "3+ unknowns coalesce into a single alert" implementation detail. The `TechArticle` JSON-LD `about` array was slimmed from technical terms (`Bonjour service discovery`, `SSDP / UPnP`, `TCP port fingerprinting`, `ARP table`, `MAC OUI lookup`, `Reverse DNS`) to user-facing topics (`Home Wi-Fi devices`, `Network scanning`, `Smart home`, `MAC address randomization`, `Trusted devices`) so even crawlers aren't shown the architecture. The `/blog/mac-randomization-ios` deep-link is preserved — that post is appropriately technical for SEO and the user-facing devices page can defer there for readers who want depth.
  - `releases.astro` — the v1.0 "What's new" bullets were reworded to describe outcomes. "8-layer device discovery (Bonjour, SSDP/UPnP, TCP port fingerprinting, ARP-table MAC + OUI lookup, reverse DNS, hostname keywords)" → "see every device on your Wi-Fi, automatically named and labeled". "Survey insights engine: …single-link clustering, latency profile (median, p95, jitter)…" → "Insights report: every completed survey produces an A–F graded coverage report, named dead zones, and a personalized 'what to do next' list". "Live ISP → Router → Device topology with real-time TCP latency probes per hop, plus a Cloudflare-backed multi-stream speed test" → "a real download, upload, ping, and jitter test with a clear ISP → Router → Device picture". The "Camera feed is used by ARKit only" privacy bullet was rewritten to "Camera feed is used only to track your iPhone's motion as you walk".
  - `support.astro` — the SEO description was changed from "answers to common questions about ARKit and Local Network permissions" to "answers to common questions about Camera and Local Network permissions" (the actual underlying `Support.md` content syncs from the canonical `Support.md` at the repo root via `sync-legal.mjs`, and that doc already describes the permission to users as "Camera" — the website description was the only place still saying "ARKit").
  - `lib/site.ts` — the site-wide `description` field (the OG / Twitter description served on every share) was changed from "WiFi Buddy turns your iPhone into a walking Wi-Fi heatmap. Survey your home in AR, see exactly where the dead zones are, and get personalized fixes — plus an 8-layer device scanner that names every gadget on your network." to "WiFi Buddy turns your iPhone into a walking Wi-Fi heatmap. Find dead zones, get personalized fixes, and see every device on your network — all in one beautifully simple app." The "8-layer device scanner" framing was the single phrase most likely to leak the architecture into social-card previews.

What was deliberately **not** changed:
- The 9 evergreen blog posts under `src/content/blog/*.mdx`. They're long-form SEO content where some technical depth is appropriate, and the user's request scoped to "the wording across the website" / the marketing pages. Specifically the `mac-randomization-ios.mdx` post is still linked from `/devices` and is intentionally allowed to be more technical there — the deep-link is the seam that lets curious readers dig in without making the marketing page feel like a spec sheet.
- The release-notes "Pro & pricing" and "Privacy & platform" bullets — those describe user-facing payment behavior and privacy guarantees, not implementation, and were already appropriate.
- The legal pages (`/privacy`, `/terms`, `/support`) — content syncs from canonical Markdown at the repo root via `sync-legal.mjs`; only the `support.astro` SEO description (which lives in the page wrapper, not the synced content) was touched.
- All `Logo.astro` / `AppStoreButton.astro` / `BaseLayout.astro` SEO scaffolding (`MobileApplication` JSON-LD, canonical URLs, sitemaps). The structured data exposes pricing and a star rating which are appropriate to share publicly; it doesn't reveal any of the implementation details that the copy was leaking.

Build verification: `npm run build` passes with all 17 routes generated cleanly and no lint errors after the rewrite.

**Pre-launch TODOs (also documented in `website/README.md`):**
1. `SITE.appStoreUrl` — replace `idTODO` with the real App Store ID once the listing exists.
2. `SITE.url` and `astro.config.mjs → site` — set both to the production domain (currently `https://wifibuddy.app`).
3. `SITE.rating` — fill in App Store Connect's real numbers, or set to `null` to suppress `aggregateRating` JSON-LD until reviews land. The placeholder `value: 4.8, count: 124` only feeds the JSON-LD now (the homepage hero no longer has visible rating copy after the April 2026 refresh), so flipping `SITE.rating = null` is the only edit needed if shipping pre-rating.
4. Smart App Banner — uncomment the `apple-itunes-app` meta in `BaseLayout.astro` once you have an App Store ID. Mobile Safari then shows an "Open in App Store" banner above the page.

**Off-page work (manual, captured in the README, not code):** AlternativeTo.net listings under each competitor, ProductHunt / AppShopper / AppAdvice, Reddit threads in r/HomeNetworking + r/wifi + r/iOSProgramming, Wikipedia citation on the *Wireless site survey* article (citation only, not a Wikipedia article about the app).

**Local dev:**
```
cd website
npm install   # one-time; populates node_modules/
npm run dev   # http://localhost:4321 (also runs sync-legal first)
npm run build # produces website/dist/
npm run preview # serves the production build locally
```

`predev` and `prebuild` automatically invoke `scripts/sync-legal.mjs`, so the legal-doc copy is always fresh when running the dev server or building. The dev server has hot-reload — edit any file under `src/` and the browser refreshes automatically.

**Deployment targets** (all supported by static output): Cloudflare Pages (recommended), Vercel, Netlify, GitHub Pages with custom domain, S3 + CloudFront. After deploy, submit `/sitemap-index.xml` to Google Search Console and Bing Webmaster Tools.

**`.gitignore` updates:** the project-root `.gitignore` was extended to ignore `website/node_modules/`, `website/dist/`, `website/.astro/`, and **`website/src/content/legal/`** — the last one is critical because those legal files are auto-generated copies of the canonical Markdown at the repo root, and committing them would let them drift from the source of truth.
