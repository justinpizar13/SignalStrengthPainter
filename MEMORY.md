...
- **Gate Pro features behind `isProUser`** — the purchase flow is live but the paid features themselves aren't gated yet. Inject `ProStore` as an `@EnvironmentObject` and use `proStore.isProUser` (not the `@AppStorage` flag) as the authoritative gate on anything we want to restrict (e.g., advanced insights, unlimited surveys, device identification depth).
