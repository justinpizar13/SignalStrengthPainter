...
| **`NetworkScanner.swift`** | **Device discovery engine**: Combines **Bonjour browsing** (`NWBrowser` for service types like `_airplay._tcp`, `_http._tcp`, `_homekit._tcp`, etc.) with **TCP subnet scanning** (probes common ports 80, 443, 62078, etc. across the /24 range). Uses `getifaddrs` to determine local IP/mask. Classifies devices into types (router, phone, computer, smart TV, printer, speaker, IoT, game console) based on Bonjour names and services. After scanning, performs **reverse DNS resolution** (`getnameinfo`) on all discovered IPs to resolve hostnames (e.g., "Johns-MacBook-Pro.local"). Resolved hostnames also feed back into device classification for previously-unknown devices. Supports **trusted device** marking: `setTrusted(_:trusted:)` persists trusted IPs in `UserDefaults`; trusted state is restored on subsequent scans. Scans in batches of 30 with 300 ms timeout per probe. Published properties: `devices`, `isScanning`, `scanProgress`, `localIP`, `subnetMask`, `scanStatusMessage`. |
| **`DeviceDiscoveryView.swift`** | **Devices tab**: "Who's on my Wi-Fi?" experience. Shows network info card (IP, subnet, device count), scan button, live progress bar during scan, scrollable device list with type icons, latency badges, resolved hostnames, and **"TRUSTED" badge** for trusted devices. Tappable rows open a detail sheet. **Security assessment** card (Looks Good / Review / Attention) now counts only **untrusted unknown** devices — trusted devices are excluded and shown in a separate "Trusted" stat. **Protect Your Network** tips section. `DeviceDetailSheet` shows resolved hostname as "Device Name", trust status badge, and a **Trust / Remove Trust** button with confirmation dialog. Trust state persists across scans via `UserDefaults`. |
...
- **DeviceDiscoveryView (Devices tab):** Scans the local network for connected devices using Bonjour + TCP subnet probing + **reverse DNS hostname resolution**. Shows device list with type classification, **resolved device names**, response times, network info summary, **trusted device badges**, security assessment (rates network safety based on **untrusted** unknown device count), and security tips. Tapping a device opens a detail sheet with IP, hostname, latency, services, timestamps, and a **Trust / Remove Trust** button. Trusted devices persist across scans in `UserDefaults`. `NetworkScanner` manages all discovery state.
