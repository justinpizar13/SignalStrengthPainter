...
| **`SpeedTestManager.swift`** | Multi-phase speed test engine: **latency** (10 HTTP pings, trimmed mean + jitter), **download** (8 concurrent async streams fetching 4 MB chunks from `speed.cloudflare.com/__down` via `URLSession.data(for:)` in a `TaskGroup`, bytes counted by `TransferCounter`), **upload** (4 concurrent 25 MB POSTs to `speed.cloudflare.com/__up`, byte tracking via `UploadTracker` delegate `didSendBodyData`). Each phase runs up to 12 s with 250 ms sampling. Final speed is trimmed mean of samples. |
...
- **Download phase:** Uses `URLSession.data(for:)` (the same async API as the ping phase) in **8 concurrent streams** inside a `Task.detached` + `withTaskGroup`. Each stream fetches **4 MB chunks** in a loop for the full phase duration. A `TransferCounter` (thread-safe `NSLock`-guarded `Int64`) accumulates completed bytes. Cache-busting query param + `Accept-Encoding: identity` header prevent compressed/cached responses. Only HTTP 200 responses count toward the total. The main-actor sampling loop reads the counter every ~250 ms.
...
- **Upload phase:** 4 concurrent `uploadTask` POSTs of 25 MB each to `speed.cloudflare.com/__up` (`application/octet-stream`). Byte tracking via `UploadTracker` (`URLSessionTaskDelegate`) using `didSendBodyData` delegate, per-task totals summed. Early-exit requires ≥ 2 s elapsed to avoid zero-sample results on fast connections.
- **Duration cap:** Each phase runs up to **12 seconds** then cancels remaining tasks/streams.
- **Final speed:** Trimmed mean (drop top/bottom 10% of samples).
- **Helper classes:** `TransferCounter` (download byte accumulator), `UploadTracker` (upload delegate).
